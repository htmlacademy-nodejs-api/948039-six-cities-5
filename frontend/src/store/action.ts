...
import { adapterComments, adapterOffer, adapterOffers } from '../utils/adapters/adapters-to-client';
...
export const fetchPremiumOffers = createAsyncThunk<Offer[], string, { extra: Extra }>(
  ...
  async (cityName, { extra }) => {
    const { api } = extra;
    const { data } = await api.get<OfferRdo[]>(`${ApiRoute.Offers}?city=${cityName}`);
